...
1. Добавлен явный выбор финальной архитектуры по validation objective `macro_f1 + 0.03 * high_recall`. Это убирает ситуацию, когда сохраненный пакет использует `sector_overlay`, хотя на validation и test лучше работает `ann_plus_regime`.
2. Выбранная архитектура сохраняется в `model_package.joblib`, а команда `predict` использует ее при инференсе.
3. В зависимости добавлен `pyarrow`, чтобы parquet-входы и выходы из README работали без CSV fallback.
4. Подготовлены графики для комиссии: сравнение архитектур, ablation финотчетности, confusion matrix, classwise-метрики, feature importance, walk-forward и drift.
...
- Validation selection score без отчетности: **0.7035**.
- Validation selection score с отчетностью: **0.7109**.
...
Финальная архитектура `ann_plus_regime` выбрана по validation, а не по test: ее validation selection score равен **0.7163**, против **0.7109** у enriched-reference.
...
На validation я сравнил модель без отчетности и с отчетностью. Selection score вырос с 0.7035 до 0.7109, macro-F1 - с 0.6770 до 0.6848. Поэтому validation-gate выбрал вариант with_reports.

Финальная архитектура выбиралась по validation-метрике, где macro-F1 дополняется небольшим бонусом за recall класса high. Победила архитектура `ann_plus_regime`. На test она дала macro-F1 0.5494, balanced accuracy 0.5714, high-risk recall 0.9062 и high-risk false negative rate 0.0938.
...
1. Этот файл: `results/defense_run/DEFENSE_MATERIALS.md`.
2. Графики из папки `results/defense_run/defense_assets/`.
3. Полные метрики: `results/defense_run/metrics.json`.
4. Предсказания test-периода: `results/defense_run/predictions.csv`.
5. Сохраненный пакет модели: `results/defense_run/model_package.joblib`.
...
risk-pipeline --config configs/config.example.yaml \
  ...
  --out results/defense_run
.venv/bin/python scripts/make_defense_materials.py results/defense_run
```
